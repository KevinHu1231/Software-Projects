...
class KalmanFilter(object):
    ...
    def predict(self,u):
        if self.phi>-100 and self.phi<100:
            self.Dmatrix = self.h/((self.h)**2+(self.d-self.x)**2)
            self.x_est = self.x + self.u*self.T
            if self.x<=2.5:
                self.phi_est = math.atan(self.h/(self.d-self.x))
            else:
                self.phi_est = math.atan(self.h/(self.d-self.x)) + math.pi

            self.P_est = self.P + self.Q

            rospy.loginfo("x_est: %f phi_est: %f P_est: %f phi: %f", self.x_est, self.phi_est, self.P_est, self.phi)
        else:
    ...
    def measurement_update(self):
        if self.phi>-100 and self.phi<100:
            self.Smatrix = self.Dmatrix*self.P_est*self.Dmatrix + self.R
            self.Wmatrix = self.P_est*self.Dmatrix/self.Smatrix
            self.P = self.P_est - self.Wmatrix*self.Smatrix*self.Wmatrix
            self.x = self.x_est + self.Wmatrix*(self.phi - self.phi_est)
            rospy.loginfo("P: %f x: %f", self.P, self.x)
        return
...
if __name__=="__main__":
    ...
    try:
        ...
        ax2.set(xlabel='time (s)', ylabel='covariance (m^2)',
            title='Covariance')
        fig2.savefig("covariance_ofilter.png")
        print(e)
